...
def find_nn_cos(v, Wv, k=10):
    ...
    pass
    #### YOUR CODE HERE ####
    similarities = np.divide(np.dot(Wv,v),(np.multiply(np.linalg.norm(Wv, axis=1),np.linalg.norm(v))))
    #nns = np.argsort(similarities)[:k]
    nns = np.argsort(similarities)[-k:][::-1]
    ds = similarities[nns]
    return (nns, ds)
...
def analogy(vA, vB, vC, Wv, k=5):
    ...
    #### YOUR CODE HERE ####
    anal_vect = vB-vA+vC
    return find_nn_cos(v=anal_vect, Wv=Wv, k=k)
